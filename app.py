...
def generate_suricata_rule_cowrie(parsed_data, initial_sid):
    ...
    for data in parsed_data:
        ...
        rule = f'alert tcp {source_ip}:any -> {destination_ip}:22 (msg:"Suspicious activity detected: Login attempt with username: \\"{escaped_username}\\" and password: \\"{escaped_password}\\"; sid:{sid}; rev:1; classtype:attempted-user;)'
        rules.append(rule)
        sid += 1
    return rules
...
def generate_suricata_rule_honeytrap(parsed_data, initial_sid):
    ...
    for data in parsed_data:
        ...
        rule = f'alert tcp {source_ip}:any -> {destination_ip}:any (msg:"Suspicious activity detected: Unauthorized access attempt with {bytes_count} bytes"; sid:{sid}; rev:1; classtype:attempted-recon;)/n'
        rules.append(rule)
        sid += 1
    return rules
